...
ggplot(combined.counts[combined.counts$condition.x == "PA" | combined.counts$condition.x == "SETD2KO"],aes(x=condition.x,y=ratio)) +
  geom_boxplot(show.legend = FALSE,aes(fill=condition.x)) +
  geom_jitter(mapping=aes(x=condition.x,y=ratio,fill=condition.x),data = combined.counts[combined.counts$condition.x == "PA" | combined.counts$condition.x == "SETD2KO"],show.legend = FALSE,size=0.7) +
  labs(x="",y="Exon/Intron H3K36me1 signal")  +
